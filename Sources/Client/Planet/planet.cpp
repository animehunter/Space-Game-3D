...
Planet::Planet(CL_GraphicContext &gc)
: num_triangles(0)
{
	create(gc);
}
...
void Planet::render(CL_GraphicContext &gc, const LightModel &light_model)
{
...
CL_Vec4f Planet::fractal(float x0, float y0, CL_Vec4f p)
{
	return p + fractal((x0+p.x)*1000.0f, (y0+p.y)*1000.0f)/30.0f;
}

int Planet::fractal(float x0, float y0)
{
	float x = 0;
	float y = 0;
...
void Planet::create(CL_GraphicContext &gc)
{
	Sphere sphere;
	...
	CL_String resource_dir = "Resources/Planet/"; // CL_Directory::get_resourcedata(app_name);
	CL_VirtualDirectory vd;
	program_object = CL_ProgramObject::load(gc, resource_dir + L"planet_vertex.glsl", resource_dir + L"planet_fragment.glsl", vd);
	program_object.bind_attribute_location(0, "in_position");
	program_object.bind_attribute_location(1, "in_normal");
	if (!program_object.link())
		throw CL_Exception("Unable to link planet program object");

	texture = CL_Texture(gc, cl_texture_cube_map);
	texture.set_cube_map(
		load_image("Resources/Planet/ground.png"),
		load_image("Resources/Planet/ground.png"),
		load_image("Resources/Planet/ground.png"),
		load_image("Resources/Planet/ground.png"),
		load_image("Resources/Planet/ground.png"),
	...
	material.ambient = CL_Vec4f(1.0, 1.0, 1.0f, 1.0f);
...
void Planet::setup_light(CL_ProgramObject &program, const Material &material, const LightModel &light_model)
{
/*
...
CL_PixelBuffer Planet::load_image(const CL_String &filename)
{
